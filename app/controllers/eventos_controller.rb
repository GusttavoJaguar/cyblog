...
class EventosController < ApplicationController
  ...
  def create
    ...
    puts "Params: #{evento_params}"
    puts "Evento valid? #{@evento.valid?}"
    puts "Evento errors: #{@evento.errors.full_messages}"
  ...
  private
    ...
    def set_evento
     @evento = Evento.friendly.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def evento_params
      params.require(:evento).permit(:nome, :descricao, :local, :data, :link_ingresso, :imagem_evento)
    end
end
